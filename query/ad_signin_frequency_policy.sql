...
signin_frequency_enabled as (
  ...
  where
    (p.users - > 'includeRoles'):: jsonb ? | (a.rid) esql
    and p.sign_in_frequency -> 'isEnabled' = 'true'
    and p.persistent_browser -> 'isEnabled' = 'true'
...
select
  ...
  case
    when (
      ...
      where
        tenant_id = t.tenant_id
    ) > 0 then 'sign-in frequency policy enabled.'
    else 'sign-in frequency policies disabled.'
  end as reason,
  t.display_name
from
  tenant_list as t;